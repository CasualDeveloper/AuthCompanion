...
scratch_root=$(mktemp -d "${TMPDIR:-/tmp}/authcompanion-release.XXXXXX")
cleanup() {
...
for architecture in arm64 x86_64; do
    ...
    bin_path=$($swift_executable build \
        ...
        --arch "$architecture" \
        --show-bin-path)
    cp "$bin_path/authcompanion" "$scratch_root/authcompanion-$architecture"
done
...
lipo -create \
    "$scratch_root/authcompanion-arm64" \
    "$scratch_root/authcompanion-x86_64" \
    -output "$scratch_root/authcompanion"
chmod 0555 "$scratch_root/authcompanion"
codesign --force --sign - --options runtime --timestamp=none "$scratch_root/authcompanion"
codesign --verify --strict "$scratch_root/authcompanion"

version_output=$($scratch_root/authcompanion --version)
version=${version_output#authcompanion }
[ "$version_output" = "authcompanion $version" ] && \
...
cp "$scratch_root/authcompanion" "$package_root/bin/"
chmod 0555 "$package_root/bin/authcompanion"
cp "$repository_root/LICENSE" "$repository_root/README.md" \
    "$repository_root/SECURITY.md" "$package_root/"
(
    cd "$package_root"
    shasum -a 256 bin/authcompanion > SHA256SUMS
)
